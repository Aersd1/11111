...
def parse_response(document):
    try:
        ...
        if not isinstance(answer, str) or not answer.strip():
            raise ModelResponseError('模型未返回正文，可能仅生成了推理内容；请提高输出上限或更换模型。')
        ...
        try:
            ...
            while exc.msg == 'Invalid \\escape':
                ...
                raise ModelResponseError('模型返回的 JSON 格式无效，请重试或更换模型。') from None
        if not isinstance(result, dict):
            raise ModelResponseError('模型返回的 JSON 必须是对象，请重试。')
